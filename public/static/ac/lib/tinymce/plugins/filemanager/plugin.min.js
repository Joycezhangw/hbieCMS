/**
 * 富文本框附件管理
 * @author joycezhang <zhangwei762@163.com>
 */
tinymce.PluginManager.add('filemanager', function(editor, url) {
    var pluginName='文件管理';
    window.tinyMceFilemanager={}; //扔外部公共变量，也可以扔一个自定义的位置
    tinyMceFilemanager.file_manager_url = editor.getParam('file_manager_url', false );
    tinyMceFilemanager.pic_arr = [];
    var openDialog = function() {
        return editor.windowManager.openUrl({
            title: pluginName,
            size: 'large',
            url:tinyMceFilemanager.file_manager_url,
            buttons: [
                {
                    type: 'cancel',
                    text: 'Close'
            ...
            onAction: function (api, details) {
                switch (details.name) {
                    case 'Ok':
                        var html = '';
                        var imgs = tinyMceFilemanager.pic_arr;
                        var len = imgs.length;
                        if(len>0) {
                            for (let i = 0; i < len; i++) {
                                if (imgs[i].file_path_url) {
                                    html += '<img src="' + imgs[i].file_path_url + '" />';
                                }
                            }
