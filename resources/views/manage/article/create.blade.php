...
@section('content')
    <form class="layui-form hb-form">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="checkbox" name="is_hot" lay-skin="switch" value="0">
            </div>
            <div class="hb-word-aux">
...
@section('javascript')
    ...
    <script>
        var SAVE_URL = "{{route('manage.article.store')}}", INDEX_URL = "{{route('manage.article.index')}}",
            HB_TAGS = [], HB_UPLOAD_URL = "{{route('manage.upload.upload')}}", CSRF_TOKEN = "{{csrf_token()}}",FILE_MANAGER_URL="{!! route('manage.attachment.album',['file_type'=>'image','select_num'=>10]) !!}";
    </script>
